...
def main(params):
    ...
    model_name, dataset_name, fold, emb_type, save_dir, load_dir = params["model_name"], params["dataset_name"], \
        params["fold"], params["emb_type"], params["save_dir"], params["load_dir"]
    ...
    with open("../configs/kt_config.json") as f:
        ...
        # 过滤 load_dir 参数
        model_config = {k: v for k, v in params.items() if k != 'load_dir'}
    ...
    if params['load_dir'] != '':
        ...
        checkpoint = torch.load(os.path.join(load_dir, "qid_model.ckpt"))
        # 获取当前模型的状态字典
